...
// convertion helpers
function convertStepsToStepSections(
  ...
  for (const step of steps) {
    if (step.isSection) {
      ...
      if (
        !(
          currentSection.title === undefined && currentSection.steps.length == 0
        )
      ) {
...
function convertIngredientsToIngredientSections(
...
export function EditRecipe() {
  ...
  const { formState, control, handleSubmit, getValues, setValue, watch } =
    useForm<EditRecipeFormInput>({
      defaultValues: async () => ({
        comments: recipe.comments,
        ingredients: recipe.ingredientSections
          .map((section) => [
            ...(section.title
              ? [{ text: section.title, isSection: true }]
              : []),
        ...
        time: {
          preparation: moment.utc(
            `${Math.floor(recipe.time.preparation / 60)}:${recipe.time.preparation % 60}`,
            "HH:mm",
          ),
          baking: moment.utc(
            `${Math.floor(recipe.time.baking / 60)}:${recipe.time.baking % 60}`,
            "HH:mm",
          ),
          total: moment.utc(
            `${Math.floor(recipe.time.total / 60)}:${recipe.time.total % 60}`,
            "HH:mm",
          ),
        },
      }),
  ...
  // split a step field into multiple fields based on linebreaks
  const splitIngredients = useCallback(
    (index: number) => {
  ...
  const handleIngredientsDrag: OnDragEndResponder = ({
    source,
    destination,
  }) => {
    if (destination) {
  ...
  const onSubmit = async (data: EditRecipeFormInput) => {
    ...
    const ingredientSections = convertIngredientsToIngredientSections(
      data.ingredients,
    );
    ...
    const recipeToSave: Recipe = {
      ...
      time: {
        preparation: moment
          .duration(data.time.preparation.format("HH:mm"))
          .asMinutes(),
                ...
                {currentTabIndex == 0 ? (
                  <>
                    ...
                    <Autocomplete
                      ...
                      value={tagsFields.map((field, index) => ({
                        name: watch(`tags.${index}`).name,
                      }))}
                      options={tags?.map((tag) => ({ name: tag.name })) ?? []}
                      getOptionLabel={(option) =>
                        (option as { name: string }).name
                      } // strange behaviour with typescript
                      onChange={(event, newValues) => {
                        ...
                        const newValueObjects = newValues.map((value) =>
                          typeof value === "string"
                            ? { name: sanitizeTagName(value) }
                            : value,
                        );
              ...
              <SpeedDialAction
                ...
                FabProps={{
                  sx: {
                    ...
                    borderColor:
                      index == currentTabIndex ? "secondary.main" : "inherit",
                  },
                }}
