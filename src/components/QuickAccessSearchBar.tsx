    ...
    const options: Option[] | undefined = useLiveQuery(async () => {
        if (currentSearch.startsWith("#")) {
            ...
            const matchingRecipes =
                (await database.recipes.orderBy("name")
                    .filter(recipe => recipe.name.toLowerCase().includes(recipeSearch))
                    .toArray()
                );

            return matchingRecipes.map(recipe => ({ kind: "recipe", payload: recipe }));
        }
    }, [currentSearch]);
        ...
        <Grid container>
            ...
            <Grid item xs={11} md={4}>
                <Autocomplete
                    ...
                    options={options ?? []}
                    getOptionLabel={option => option.kind === "tag" ? "#" + option.payload.name : option.payload.name}
